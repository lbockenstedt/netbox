...
DCIM/IPAM: `NETBOX_HEALTH`, `NETBOX_GET_SITES`, `NETBOX_GET/ADD/UPDATE/DELETE_RACK`, `NETBOX_GET/ADD/CLAIM/DELETE/UPDATE_DEVICE`, `NETBOX_GET_DEVICE_FORM_OPTIONS`, `NETBOX_GET_PREFIXES`, `NETBOX_ALLOCATE_PREFIX`, `NETBOX_FIND_AVAILABLE_PREFIXES`, `NETBOX_CLAIM/UPDATE/DELETE_PREFIX`, `NETBOX_GET_IPS`, `NETBOX_ALLOCATE/RELEASE/UPDATE_IP(_ADDR)`, `NETBOX_DOC_VM`, `NETBOX_GET_TENANTS`, `NETBOX_GET_TENANT_GROUPS`, `NETBOX_SEARCH`.

> **Tenant groups** — `NETBOX_GET_TENANT_GROUPS` returns each NetBox tenant group with its **transitive** membership (`tenant_slugs`: its own tenants *plus* every tenant of a descendant group — membership rolls UPWARD; a child never inherits its parent's tenants). The hub stores each group as its own selectable tenant keyed `group:<slug>` (flagged `is_tenant_group`), so a user can be assigned to one tenant *or* to a whole group; selecting the group shows the union. `NETBOX_GET_RACKS/DEVICES/PREFIXES/IPS` accept an optional `tenant_group` slug that **supersedes** `tenant` and maps to NetBox's tree-aware `?tenant_group=` filter (the union happens server-side, so no slug list is sent — NetBox rejects an entire query if one slug is unknown). Writes still require a real tenant: a group-assigned user may create into any *member* tenant, never into the group itself. `NETBOX_GET_TENANTS` now also tags each tenant with its owning `group_slug`/`group_name`.
...
- `NETBOX_GET_RACK_ELEVATION` — `get_rack_elevation(rack_id)` (read-only render model): calls NetBox's `/api/dcim/racks/{id}/elevation/?face=front|rear` (one entry per RU, top→bottom, multi-U devices in consecutive units) + the rack's device list for 0U/side devices (position null/0), and returns `{rack:{name,u_height,site,tenant}, faces:{front,rear}, zero_u:[…]}` with per-device summaries carrying the role `color` for slot tinting. NOT a picklist mutation. WebUI IPAM → Racks → eye button; non-admins ownership-gated at the hub route.
...
- `src/netbox_dcim.py` (`DcimMixin`) — sites, racks, devices, `update_device_ip`, form-options picklists, `seed_catalog()` (loads `src/seed_catalog.json`), the Excel rack-layout importer: `_resolve_device_type_slug(model)` (catalog stem + port-hint resolver) + `import_rack_layout(selected, dry_run)`, and `get_rack_elevation(rack_id)` (front+rear render model for the WebUI elevation view; `_elev_devsum`/`_name_of` helpers).
